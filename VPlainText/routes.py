...
from File_data import (
    read_fingerprint, read_matricula,
    write_fingerprint, fingerPrintFile, matriculaFile
)
import time
...
def registrar_rutas(app, blockchain, sistemaBiometrico):
    ...
    def simulate_blocks(num_blocks):
        tiempos = []
        matricula, fingerprint_data = read_fingerprint(fingerPrintFile)
        if fingerprint_data is None or matricula is None:
            return jsonify({'message': 'No se pudo obtener datos desde el archivo txt.'}), 404
    ...
    def get_block():
        update()
        matricula = read_matricula(matriculaFile)
        if not matricula:
        ...
        if all(dato.values()):
            ...
            write_fingerprint(fingerPrintFile, matricula, dato_biometrico)
